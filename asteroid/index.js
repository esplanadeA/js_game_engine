...
function scoreCount() {
  ...
  for (let i = asteroids.length - 1; i >= 0; i--) {
    ...
    for (let j = projectiles.length - 1; j >= 0; j--) {
      ...
      // Check if the projectile collides with the asteroid
      if (circleCollision(asteroid, projectile)) {
        // If there is a collision, increment the score and remove the asteroid and projectile
        score++;
        asteroids.splice(i, 1);
        projectiles.splice(j, 1);
      }
  ...
  console.log('Score:', score); // Output the score
}
